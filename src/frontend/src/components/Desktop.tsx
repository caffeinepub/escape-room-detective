...
const desktopItems: DesktopItem[] = [
  ...
  },
  {
    id: "2",
    name: "Evidence",
    customIcon: "/assets/uploads/image-1.png",
    type: "folder",
    description: "Physical and digital evidence",
  },
  {
    id: "3",
  ...
  {
    id: "7",
    name: "Rješenje Slučaja",
    icon: Scale,
    type: "folder",
    description: "Suspect identification",
  },
];
...
export default function Desktop({ audioRef }: DesktopProps) {
  ...
  const [localGamePhase, setLocalGamePhase] = useState<
    "playing" | "won" | "lost"
  >("playing");
  ...
  useGetGamePhase();
  const { data: remainingSeconds } = useGetRemainingSeconds();

  // Monitor timer and detect when time runs out
  useEffect(() => {
    if (localGamePhase === "playing" && remainingSeconds !== undefined) {
      const seconds = Number(remainingSeconds);
      if (seconds <= 0) {
        setLocalGamePhase("lost");
      }
    }
  }, [remainingSeconds, localGamePhase]);

  const isGameComplete = localGamePhase === "won" || localGamePhase === "lost";
  ...
  return (
    <div className="min-h-screen w-full bg-background animate-boot relative">
      ...
      {localGamePhase === "won" && <VictoryScreen audioRef={audioRef} />}
      {localGamePhase === "lost" && <DefeatScreen audioRef={audioRef} />}
      ...
      <div
        ...
        style={{
          backgroundImage:
            "url(/assets/generated/detective-desktop-bg.dim_1920x1080.jpg)",
      ...
      <div className="relative z-10">
        ...
        {showEvidence && (
          <EvidenceGallery onClose={() => setShowEvidence(false)} />
        )}
                ...
                <Card className="max-w-md border-destructive/50 bg-destructive/5">
                  <CardHeader>
                    ...
                    <CardDescription className="font-mono text-sm">
                      Your device does not support camera access through the
                      browser.
                    </CardDescription>
                  </CardHeader>
                </Card>
                  ...
                  <Card className="border-accent/50 bg-accent/5">
                    <CardContent className="py-4">
                      <p className="text-sm text-center font-mono text-muted-foreground">
                        Position camera to document crime scene evidence. All
                        captures are logged with timestamp and case number.
                      </p>
                    </CardContent>
                  </Card>
                </div>
